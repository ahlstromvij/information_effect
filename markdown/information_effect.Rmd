...
How does this work in practice? That's the question this guide is looking to answer. It will walk through each step in a complete pipeline from constructing a political knowledge scale from a set of items to modeling the relevant effects, using functions written in  the R programming language [@r_core_team] -- a free, open source language for statistical computing -- that can be re-used by others interested in information effects modeling on their own particular data sets.
...
Let's start by fitting a model on our four political knowledge items from the BES data set, which are as follows:
